...
async def test_create_resource_version(test_client: AsyncClient):
    ...
    resp = await test_client.post(
        f"/api/v1/versions/resources/{resource_id}/versions",
...
async def test_create_version_nonexistent_resource(test_client: AsyncClient):
    ...
    resp = await test_client.post(
        "/api/v1/versions/resources/nonexistent-res/versions",
...
async def test_list_resource_versions(test_client: AsyncClient):
    ...
    resp = await test_client.get(f"/api/v1/versions/resources/{resource_id}/versions")
    assert resp.status_code == 200
    data = resp.json()
    assert data["resource_id"] == resource_id
    assert data["total"] == 2
...
async def test_create_snapshot(test_client: AsyncClient):
    ...
    resp = await test_client.post("/api/v1/versions/snapshots", json={
        "name": "test-snapshot",
...
async def test_list_snapshots(test_client: AsyncClient):
    ...
    await test_client.post("/api/v1/versions/snapshots", json={
        "name": "snap-1",
    })

    resp = await test_client.get("/api/v1/versions/snapshots")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_get_snapshot(test_client: AsyncClient):
    ...
    create_resp = await test_client.post("/api/v1/versions/snapshots", json={
        "name": "snap-get-test",
    })
    snapshot_id = create_resp.json()["snapshot_id"]

    resp = await test_client.get(f"/api/v1/versions/snapshots/{snapshot_id}")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_delete_snapshot(test_client: AsyncClient):
    ...
    resp = await test_client.delete(f"/api/v1/versions/snapshots/{snapshot_id}")
    assert resp.status_code == 200
    assert resp.json()["deleted"] is True

    resp2 = await test_client.get(f"/api/v1/versions/snapshots/{snapshot_id}")
