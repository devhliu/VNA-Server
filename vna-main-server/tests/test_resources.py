...
async def test_list_resources(test_client: AsyncClient):
    ...
    resp = await test_client.get("/v1/resources")
...
async def test_create_resource(test_client: AsyncClient):
    ...
    resp = await test_client.post("/v1/resources", json=payload)
...
async def test_get_resource(test_client: AsyncClient):
    ...
    create_resp = await test_client.post("/v1/resources", json={
        "data_type": "dicom",
        "source_type": "dicom_only",
    ...
    resp = await test_client.get(f"/v1/resources/{resource_id}")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_get_resource_not_found(test_client: AsyncClient):
    ...
    resp = await test_client.get("/v1/resources/nonexistent-id")
...
async def test_update_resource(test_client: AsyncClient):
    ...
    resp = await test_client.put(f"/v1/resources/{resource_id}", json={
...
async def test_delete_resource(test_client: AsyncClient):
    ...
    resp = await test_client.delete(f"/v1/resources/{resource_id}")
    assert resp.status_code == 200
    assert resp.json()["deleted"] == resource_id

    resp2 = await test_client.get(f"/v1/resources/{resource_id}")
...
async def test_resource_with_patient(test_client: AsyncClient):
    ...
    # Create patient
    pat_resp = await test_client.post("/v1/patients", json={
        "hospital_id": "HOSP-R1",
        "source": "manual",
    })
    ...
    pat_resources = await test_client.get(f"/v1/patients/{patient_ref}/resources")
