...
async def test_query_all_resources(test_client: AsyncClient):
    await test_client.post("/api/v1/resources", json={
...
async def test_query_by_source_type(test_client: AsyncClient):
    ...
    resp = await test_client.post("/api/v1/query", json={"source_type": "bids_only"})
    data = resp.json()
...
async def test_query_by_dicom_uid(test_client: AsyncClient):
    ...
    resp = await test_client.post("/api/v1/query", json={"dicom_study_uid": "1.2.3.4.5"})
    data = resp.json()
    assert data["total"] >= 1
...
async def test_query_by_bids_subject(test_client: AsyncClient):
    ...
    resp = await test_client.post("/api/v1/query", json={"bids_subject_id": "sub-42"})
    data = resp.json()
    assert data["total"] >= 1
...
async def test_query_by_labels(test_client: AsyncClient):
    resp = await test_client.post("/api/v1/resources", json={
        "source_type": "dicom_only",
        "data_type": "dicom",
    })
    rid = resp.json()["resource_id"]
    await test_client.put(f"/api/v1/labels/resource/{rid}", json={
        "labels": [
...
async def test_query_text_search(test_client: AsyncClient):
    ...
    resp = await test_client.post("/api/v1/query", json={"text_search": "chest"})
    data = resp.json()
    assert data["total"] >= 1
...
async def test_query_by_hospital_id(test_client: AsyncClient):
    ...
    resp = await test_client.post("/api/v1/query", json={"hospital_id": "H-QUERY-999"})
    data = resp.json()
    assert data["total"] >= 1
...
async def test_query_pagination(test_client: AsyncClient):
    ...
    resp = await test_client.post("/api/v1/query", json={"offset": 0, "limit": 5})
    data = resp.json()
